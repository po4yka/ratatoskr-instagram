...
- Publish the ratified local-removal fact `social.source.removed.v1` with each capture tombstone, so Knowledge can delete the corresponding indexed document without treating it as an upstream Instagram deletion.
- Consume the published typed `knowledge.analysis.completed.v1` fact through an inbox and retain only an observational capture/digest/completion-time link. The captured/updated preservation facts themselves are the analysis request flow; no Instagram command channel or analysis payload is introduced.
...
- Schema: the current development schema adds local capture tombstones and capture-analysis linkage records. This is an in-place current-schema definition, not a migration.
- Downstream: Knowledge receives captured/updated/removed facts and emits the typed completion linkage fact; its analysis and index internals remain out of scope.
