...
// ordinal 05 belongs to this repository.
const SCHEMA_LOCK: i64 = 0x7261_7461_736b_7205;
...
pub enum PersistenceError {
    ...
    #[error("the instagram_archive schema could not be applied")]
...
/// One finite database pool owned by the Instagram bounded context.
#[derive(Debug, Clone)]
pub struct Database {
    pool: PgPool,
}
...
impl Database {
    ...
    /// `instagram_archive` exists, and applies the file only if it does not.
...
async fn lock_and_apply(connection: &mut sqlx::PgConnection) -> Result<(), sqlx::Error> {
    ...
    let present: Option<String> =
        sqlx::query_scalar("select to_regnamespace('instagram_archive')::text")
